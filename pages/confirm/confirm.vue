	...
	<view class="confirm">
		...
		<view v-if="notXuzuFeature()" class="quanpinquanjiao address fxpad" @click="umenPlus1()">
			...
			<view v-else class="left2">
				<image style="width: 48rpx; height: 48rpx; margin-right: 24rpx"
					src="https://oss.chuangliukeji.com/backstage/2024-04-25/e0942d72f3474a48a3b413abbaac77f3.png"
		...
		<view class="quanpinquanjiao timeView">
			<view class="timeView_div">
				<image src="https://oss.chuangliukeji.com/backstage/2024-04-25/5fc11335f3f54cb6a65a01f34509cf70.png">
				</image>
			...
			<view class="timeView_div">
				<image src="https://oss.chuangliukeji.com/backstage/2024-04-25/9df91c29c5bf4f2aac10c25f82bf0848.png">
				</image>
				<view class="timeView_div_content">
					<view>预期归还日</view>
		...
		<view class="container quanpinquanjiao">
			<view class="item fxpad pddingB0">
				<view class="fxpan">
					...
					<text class="line_text">商品押金</text>
				</view>
			</view>
			<!-- <view class="item fxpad pddingB0 paddingT20"> -->
			<view class="item fxpad paddingT20" style="padding-bottom: 20rpx;">
				<view class="left_type1">
					<view class="type_fxpat">
						<image src="https://oss.chuangliukeji.com/backstage/2024-09-14/e58b76cf4d924120977e677c1207331e.png" class="fxpat_img"></image>
						<text>智能风控免押</text>
					</view>
					<view class="type_fxpat">
						<!-- <text style="color: #999999;">芝麻信用免押未通过请选择此项</text> -->
						<text style="color: #999999;">平台智能免押服务</text>
					</view>
				</view>
				<image v-if="yaJinType == 'mianYa' " src="/static/orderList/inactive.png" @click="onClickYaJinType('zhiNeng')" :class="{
				  type_checkIcon: true,
				}" />
				<image v-if="yaJinType == 'zhiNeng' " src="/static/orderList/active.png" :class="{
				  type_checkIcon: true,
				}" />
			</view>
			<!-- <view class="item fxpad paddingT20">
				<view class="left_type1">
					<view class="type_fxpat">
						<image src="https://oss.chuangliukeji.com/backstage/2024-09-14/acb5176538a44cdbaefdac79295edc8e.png" class="fxpat_img"></image>
						<text>芝麻信用免押（推荐）</text>
					</view>
					<view class="type_fxpat">
						<text style="color: #999999;">芝麻分600分以上有机会全额免押</text>
					</view>
				</view>
				<image v-if="yaJinType == 'zhiNeng' " src="/static/orderList/inactive.png" @click="onClickYaJinType('mianYa')" :class="{
				  type_checkIcon: true,
				}" />
				<image v-if="yaJinType == 'mianYa' " src="/static/orderList/active.png" :class="{
				  type_checkIcon: true,
				}" />
			</view> -->
		</view>
		...
		<view class="container quanpinquanjiao">
			...
			<view class="item fxpad pddingB0" style="padding-bottom: 24rpx;"
				v-if="orderPrices.otherPeriodsPrice && notXuzuFeature()">
		...
		<XieYiComp v-if="productId || xuzuGlobalData.detailDto.productId" :show="showXieyiModal" :productId="productId || xuzuGlobalData.detailDto.productId" :xYReqParams="xYData" :servicesPrice="servicesPrice" @hide="showXieyiModal = false" @ok="xieyiOkHandler" />
	</view>
</template>
...
<script>
	...
	export default {
		...
		onShow(e) {
			...
			if(currPage.data.isSimple){
				that.hasCertification = true;
				console.log("二要素回来的》》》》》");
				that.onOk();
				currPage.data.isSimple = false;
			};
			// 更新实名认证信息
			setTimeout(function() {
		...
		methods: {
			...
			onConfirm() {
				...
				this.$request(
					...
					this.servicesPrice = res.data.additionalServices.map(
					    (item) => item?.shopAdditionalServices?.price
					);
			...
			comSrvicesPrice() {
				...
				let list = [];
				service.map(item => {
					if(servicesId.includes(item.shopAdditionalServicesId)){
						list.push(item.shopAdditionalServices.price)
					}
				})
				this.servicesPrice = list;
			},
			getServicesData(val) {
					...
					if (res.data.freezeUrl) {
						tradePay(res.data.freezeUrl).then((res) => {
							...
							if (res.resultCode === "9000") {
								//跳商品详情页
								_this.$onTo(
									`pages/orderDetail/orderDetail?orderId=${_this.productDescDetail.orderId}`,
									false, true);
							} else {
								// 取消支付或错误
								_this.cancelOrderTradePlay();
							}
						});
					}else {
						// 走智能风控免押，没有支付url，不拉起支付
						uni.hideLoading()
						_this.$eventEmitter.emit(
						"refreshMyPageConfigData"); // 刷新我的页面的配置数据，使得我的订单上的气泡上的数字会显示相应的值
			...
			// 取消支付后续处理
			cancelOrderTradePlay() {
				const yaJinTypeData = this.yaJinType;
				const _this = this;
				if(yaJinTypeData === 'mianYa') {
					//免押取消订单支付，直接调用关单接口
					_this.$request(
			...
			generateOrderSubmitUrlAndData() {
				...
				if (this.isXuzu) {
					...
					// 如果选的芝麻免押，则走交易组件代扣，需传入sourceId
					if(this.yaJinType === 'mianYa') {
